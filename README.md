...
#### 2. 사용자한테서 숫자 3개를 입력받는다. (1부터 9까지 입력가능, 중복 x)
#### 3. 생성된 난수와 입력받은 숫자를 비교한다.
#### 4. 숫자와 위치가 같을 경우 Strike.
#### 5. 숫자는 같지만 위치가 틀릴 경우 Ball.
#### 6. 못맞췄을 경우 게임이 끝나지 않고 카운트가 증가함. (최대 10번)
...
#### 넘겨받은 값과 사용자가 입력하여 저장할 배열을 만들어줌.
...
#### result값이 2이하면 "최상", 5이하면 "보통" 8이하면 "하", 그 이상일 경우 "최하".
